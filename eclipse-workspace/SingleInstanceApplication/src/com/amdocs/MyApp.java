...
public class MyApp {
	...
	private void updateAppRunningStatus(boolean appStatus) {
		...
		try {
		BufferedWriter bufferedWriter = new BufferedWriter (new FileWriter("application.txt "));
		bufferedWriter.write("" + appAlreadyRunning);
		bufferedWriter.close();
	}
		catch(Exception e) {
			System.out.println("Error - Unable to write to the application.txt file");
			e.printStackTrace();
		}
	}

	private void isAppRunning() throws AppAlreadyRunningException {
		try {		
